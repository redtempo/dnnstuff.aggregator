...
jQuery.fn.codaSlider = function(settings) {
	...
	return this.each(function(){
		...
		if (settings.dynamicArrows) {
			...
			slider.before('<div class="coda-nav-left" id="coda-nav-left-' + settings.sliderCount + '"><a href="#">' + settings.dynamicArrowLeftText + '</a></div>');
			slider.after('<div class="coda-nav-right" id="coda-nav-right-' + settings.sliderCount + '"><a href="#">' + settings.dynamicArrowRightText + '</a></div>');
		};
		...
		// Left arrow click
		jQuery("#coda-nav-left-" + settings.sliderCount + " a").click(function(){
			navClicks++;
			if (currentPanel == 1) {
		...
		// Right arrow click
		jQuery('#coda-nav-right-' + settings.sliderCount + ' a').click(function(){
			navClicks++;
		...
		if (settings.dynamicTabs) {
			var dynamicTabs = '<div class="coda-nav" id="coda-nav-' + settings.sliderCount + '"><ul></ul></div>';
			...
			ul = jQuery('#coda-nav-' + settings.sliderCount + ' ul');
		...
		jQuery('#coda-nav-' + settings.sliderCount + ' a').each(function(z) {
		...
		jQuery(settings.externalTriggerSelector).each(function() {
			...
			// DNNSTUFF if (settings.sliderCount == parseInt(jQuery(this).attr("rel").slice(12))) {
			if (settings.sliderCount == jQuery(this).attr("rel").slice(12)) {
				jQuery(this).bind("click", function() {
					navClicks++;
		...
		if (settings.crossLinking && location.hash && parseInt(location.hash.slice(1)) <= panelCount) {
			jQuery("#coda-nav-" + settings.sliderCount + " a:eq(" + (location.hash.slice(1) - 1) + ")").addClass("current");
		// If there's no cross-linking, check to see if we're supposed to load a panel other than Panel 1 initially...
		} else if (settings.firstPanelToLoad != 1 && settings.firstPanelToLoad <= panelCount) {
			jQuery("#coda-nav-" + settings.sliderCount + " a:eq(" + (settings.firstPanelToLoad - 1) + ")").addClass("current");
		// Otherwise we must be loading Panel 1, so make the first tab the current one.
		} else {
			jQuery("#coda-nav-" + settings.sliderCount + " a:eq(0)").addClass("current");
		};
